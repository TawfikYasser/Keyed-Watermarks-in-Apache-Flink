...
public class InternalTimerServiceImpl<K, N> implements InternalTimerService<N> {
    ...
    /** Event time timers per key that are currently in-flight. */
    private final HashMap<Object, KeyGroupedInternalPriorityQueue<TimerHeapInternalTimer<K, N>>>
            eventTimeTimersQueueKeyed;
    ...
    private ConcurrentHashMap<Object, Long> watermarksAndKeys;

    private boolean isKeyedWatermark;

    private boolean isNonKeyedWatermark;
    ...
    @VisibleForTesting
    static final String EVENT_TIMER_PREFIX = TIMER_STATE_PREFIX + "/event_keyed_";
    ...
    InternalTimerServiceImpl(
        ...
        this.priorityQueueSetFactory = checkNotNull(priorityQueueSetFactory);
        this.timerSerializer = checkNotNull(timerSerializer);
        ...
        watermarksAndKeys = new ConcurrentHashMap<>(16, 0.9f, 1);
        isKeyedWatermark = false;
        isNonKeyedWatermark = true;
        this.eventTimeTimersQueueKeyed = new HashMap<>();
    }

    /**
     * Will be used to create queues in case of keyed watermarks to store the timers of each key.
     */
    private <N>
    ...
    @Override
    public boolean isKeyedWatermark() {
        return isKeyedWatermark;
    }

    @Override
    public boolean isNonKeyedWatermark() {
        return isNonKeyedWatermark;
    }

    @Override
    public long getKeyedWatermark(Object watermarkKey) {
        return watermarksAndKeys.getOrDefault(watermarkKey, Long.MIN_VALUE);
    }

    @Override
    public void setKeyedWatermark(Object watermarkKey, long watermarkTs) {
        watermarksAndKeys.put(watermarkKey, watermarkTs);
    }
    ...
    public void registerEventTimeTimer(N namespace, long time) {
        if ((K) keyContext.getCurrentKey() != null) {
            KeyGroupedInternalPriorityQueue<TimerHeapInternalTimer<K, N>>
                    eventTimeTimerQueuePerKey =
                    eventTimeTimersQueueKeyed.get((K) keyContext.getCurrentKey());
            if (eventTimeTimerQueuePerKey == null) {
                eventTimeTimerQueuePerKey = createTimerPriorityQueue(EVENT_TIMER_PREFIX + name + (K) keyContext.getCurrentKey()+System.nanoTime(), timerSerializer);
            }
            eventTimeTimerQueuePerKey.add(
                    new TimerHeapInternalTimer<>(
                            time, (K) keyContext.getCurrentKey(), namespace));
            eventTimeTimersQueueKeyed.put(
                    (K) keyContext.getCurrentKey(), eventTimeTimerQueuePerKey);
        }
        eventTimeTimersQueue.add(
                new TimerHeapInternalTimer<>(time, (K) keyContext.getCurrentKey(), namespace));
//        if (isNonKeyedWatermark()){
//            eventTimeTimersQueueKeyed.clear();
//        }
    }
            ...
            if ((K) keyContext.getCurrentKey() != null) {
                KeyGroupedInternalPriorityQueue<TimerHeapInternalTimer<K, N>>
                        eventTimeTimerQueuePerKey =
                                eventTimeTimersQueueKeyed.get((K) keyContext.getCurrentKey());
                if (eventTimeTimerQueuePerKey != null) {
                    eventTimeTimerQueuePerKey.remove(
                            new TimerHeapInternalTimer<>(
                                    time, (K) keyContext.getCurrentKey(), namespace));
                }
            }
    ...
    public void advanceKeyedWatermark(long time, Object key) throws Exception {
        if (key != null) {
            if (key.equals(Long.MIN_VALUE)){
                // HERE WE NEED TO FIRE ALL TIMERS
                // WE WILL ITERATE OVER ALL QUEUES AND FOR EACH QUEUE WE WILL FIRE ALL TIMERS LESS THAN THE MAX WATERMARK RECEIVED
                isKeyedWatermark = true;
                isNonKeyedWatermark = false;
                InternalTimer<K, N> timer;
                for (Object keyObj : eventTimeTimersQueueKeyed.keySet()) {
                    watermarksAndKeys.put(keyObj, Long.MAX_VALUE);
                    KeyGroupedInternalPriorityQueue<TimerHeapInternalTimer<K, N>> queue = eventTimeTimersQueueKeyed.get(keyObj);
                    if (queue != null) {
                        while ((timer = queue.peek()) != null
                ...
                isKeyedWatermark = true;
                isNonKeyedWatermark = false;
                // We need to store each current watermark and its key in the hashmap, will be used by
                // WindowOperator & ProcessWindowFunction
                watermarksAndKeys.put(key, time);
                InternalTimer<K, N> timer;
                KeyGroupedInternalPriorityQueue<TimerHeapInternalTimer<K, N>>
                        eventTimeTimerQueuePerKey = eventTimeTimersQueueKeyed.get(key);
                if (eventTimeTimerQueuePerKey != null) {
                    while ((timer = eventTimeTimerQueuePerKey.peek()) != null
                            && timer.getTimestamp() <= time) {
                        keyContext.setCurrentKey(timer.getKey());
                        eventTimeTimerQueuePerKey.poll();
                        triggerTarget.onEventTime(timer);
                    }
                }
            }
