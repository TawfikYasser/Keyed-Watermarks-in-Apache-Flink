...
public class KeyedStream<T, KEY> extends DataStream<T> {
     ...
     * state by key.
     *
     * @param transformation of the keyed stream
     * @param dataStream Base stream of data
     * @param keySelector Function for determining state partitions
     */
    // Called from the `KeyedOutputStreamOperator`
    // This is KS1
    public KeyedStream(
    ...
    /**
     * The following method assignTimestampsAndKeyedWatermarks() should return
     * (KeyedOutputStreamOperator) so that we can call window() right after i.e. keyBy().window()
     *
     * <p>window() in KeyedStream and works on KeyedStream so assignTimestampsAndKeyedWatermarks()
     * should be in KeyedStream (DONE) and returns
     *
     * <p>it should be: addSource(source).KeyBy(key).assignTimestampsAndKeyedWatermarks(watermark
     * strategy) .window(i.e. reduct() ).print();
     *
     * <p>assignTimestampsAndKeyedWatermarks() will be called after keyBy() so that it will be
     * called on each stream of the keyedStream generated by keyBy() and it should return
     * KeyedOutputStreamOperator so that
     */
    public KeyedOutputStreamOperator<T, KEY> assignTimestampsAndKeyedWatermarks(
            KeyedWatermarkStrategy<T, KEY> keyedWatermarkStrategy) {
        final KeyedWatermarkStrategy<T, KEY> cleanedStrategy = clean(keyedWatermarkStrategy);
        // match parallelism to input, to have a 1:1 source -> timestamps/KeyedWatermarks
        // relationship
        // and chain

        final int inputParallelism = getTransformation().getParallelism();
        final KeyedTimestampsAndWatermarksTransformation<T, KEY> transformation =
                new KeyedTimestampsAndWatermarksTransformation<>(
                        "Timestamps/KeyedWatermarks",
                        inputParallelism,
                        getTransformation(),
                        cleanedStrategy,
                        getKeyType(),
        ...
        return new KeyedOutputStreamOperator<>(this, transformation, keySelector);
    }
}
