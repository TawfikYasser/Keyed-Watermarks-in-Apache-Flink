...
public class KeyedWatermarksWithIdleness<T, KEY> implements KeyedWatermarkGenerator<T, KEY> {

    private final KeyedWatermarkGenerator<T, KEY> watermarks;
     ...
     */
    public KeyedWatermarksWithIdleness(
            KeyedWatermarkGenerator<T, KEY> watermarks, Duration idleTimeout) {
        this(watermarks, idleTimeout, SystemClock.getInstance());
    }

    @VisibleForTesting
    KeyedWatermarksWithIdleness(
            KeyedWatermarkGenerator<T, KEY> watermarks, Duration idleTimeout, Clock clock) {
    ...
    @Override
    public void onEvent(T event, KEY k, long eventTimestamp, WatermarkOutput output) {
        watermarks.onEvent(event, k, eventTimestamp, output);
